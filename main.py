...
def finetune(run_config: RunConfig, seed):
    ...
    model_with_head = torch.compile(ModelWithHead(base_model=model, input_dim=45, hidden_dim=16).to(run_config.device))
...
def setup_dataset(run_config, token_config):
    ...
    data.load(n=run_config.n_samples, cache=Path.cwd().joinpath("cache"), use_genentech_data=False)
    data.set_data()
